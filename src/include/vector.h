...
class myVector {
 public:
  ...
  myVector() : data_(0), size_(0), capacity_(0) {};
  ...
  myVector(std::initializer_list<value_type> const &items)
  ...
  myVector(myVector &&v) : data_(v.size_), size_(v.size_), capacity_(v.capacity_) {
    v.data_ = 0;
    v.size_ = 0;
    v.capacity_ = 0;
  };
  ~myVector() { alloc_.deallocate(data_, capacity_); };
  ...
  iterator insert_many(const_iterator pos, Args &&...args) {
    ...
    // myVector<value_type> temp{std::forward<Args>(args)...};
